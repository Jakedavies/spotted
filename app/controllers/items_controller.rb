class ItemsController < ApplicationController
  ...
  def index
    ...
    if params[:search]!=nil&&params[:search]!=""
      ...
      @items = Item.order(params[:sort] + ' ' + params[:direction]).page(params[:page]).per_page(10)
    else
      #@items = Item.order('created_at DESC').paginate(:page => params[:page])
      @items = Item.order(sort_column + ' ' + sort_direction).page(params[:page]).per_page(10)
    end
  end
